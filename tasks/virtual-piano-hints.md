...
Предложенное автором решение можно усовершенствовать, использовав делегирование и сократив количество плееров на странице до одного. Также вместо свойства `event.keyCode` лучше использовать `event.Code`, возвращающее более осмысленный и понятный результат. 
...
- элемент `audio` в html не обязателен. Его можно заменить функцией `new Audio()`
...
### Как повлиять на псевдоэлементы средствами js?

Никак. Псевдоэлементы `::after` и `::before` не находятся при помощи JavaScript, им нельзя указать дополнительные свойства, например, `display: none` или `display: block`, чтобы скрыть или отобразить. Но можно повлиять на их общего родителя, указав ему дополнительный класс и уже для этого класса через `css` прописать нужные свойства псевдоэлементам. 