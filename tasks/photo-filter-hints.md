...
Необходимо отслеживать процесс загрузки изображения, событие `load`. Как это происходит и где используется можно посмотреть в [примере на codepen](https://codepen.io/irinainina/pen/abzwapy). Обратите внимание на функцию `viewBgImage`. Вместо `img.onload` желательно использовать конструкцию 
```js
